...
module.exports = (function () {
  ...
  var adapter = {
    ...
    connectConnection: function (conn, cb) {
      ...
      function registerErrorHandler(mssqlConnection) {
        mssqlConnection.on('error', function(err) {
          logError(err);
          
          // there's a connection error, so we should close this connection and let it 
      ...
      if (persistent && !connected) {
        ...
        registerErrorHandler(connection.mssqlConnection);
  ...
  function log() {
    if (adapter.loggingEnabled || adapter.debug) {
  ...
  function logError() {
    if (adapter.loggingEnabled || adapter.debug) {
        console.error("MSSQL Server adapter error:");
